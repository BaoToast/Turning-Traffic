 ...
 * 也就是說 8 支以上算得出來。
 *
 * ⚠️ **2026-09-20 使用者裁示放寬到 8**（他原本就問過要不要放寬，
 *   當時的建議是「放寬，但不要直接改成無上限」，這一輪他說「請執行」）。
 *
 *   為什麼是 8 而不是無上限：
 *   ・轉向圖的畫布**已經會跟著支線數長大**（見 traffic-app.tsx 的
 *     `extraArms * 140 / * 60`），所以 8 叉的節點與數據卡放得下——
 *     這一點是**實測過的**，見 scripts/e2e-eight-arm.mjs。
 *   ・但畫布是無限長不下去的：支線愈多，等角配置的相鄰夾角愈小，
 *     節點標籤遲早會互相擠到。與其挑一個「算得出來但看不懂」的上限，
 *     不如訂在實測確認過版面沒問題的那一格。
 *   ・實務上 8 叉已經是極少見的路口；真的遇到 9 支以上，
 *     **匯入不受這個上限限制**（見下方警語），照樣讀得進來。
 *
 * ⚠️ 放寬時**一定要重跑 scripts/e2e-eight-arm.mjs**：它量的是
 *   8 叉時節點圓心、支線標籤與數據卡有沒有互相重疊、有沒有超出 viewBox。
 *   只改常數不量版面，就是把問題留給使用者去發現。
 *
 * ⚠️ **匯入不受這個上限限制**——匯入是照調查表實際有幾支就讀幾支，
 * 不可以拿這個常數去擋匯入。擋了會讓真實資料進不來，那是嚴重得多的問題。
 */
export const MANUAL_ARM_LIMIT = 8;
