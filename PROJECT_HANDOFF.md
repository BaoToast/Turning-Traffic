...
- 帶版號的正式驗證網址：<https://baotoast.github.io/Turning-Traffic/?v=2.1.83>
- 正式 branch：`main`
- 目前正式程式版本：`v2.1.83`（2026-09-26 完成 GPT 高風險獨立複查；發布 commit 與線上證據以本文件第 12.6 節及當下 Git／GitHub 為準）
...
- `scripts/`：手冊產製、2 支種子資料產生器（`seed-state.mjs`、`make-wide-seed.mjs`）
  及 **84** 支 `e2e-*.mjs` 腳本；其中 `npm run e2e` 明列 **83** 支瀏覽器測試，
  另加那 2 支種子資料產生器，合計 **85** 個執行步驟（不含前置 GitHub build）。
  ...
  ⚠️ 報告時不得把那 85 個步驟全部稱為瀏覽器 E2E。
  ⚠️ **這三個數字每次新增／移除 e2e 腳本都要跟著改。** 2026-09-23 的複查抓到
  它們停在 83／82／84——v2.1.82 新增的 `e2e-trend-order.mjs` 已經掛進
  `npm run e2e`，這一行卻沒跟著更新，而同一份文件又要求別人照這個數字核對。
  核算方式：`ls scripts/e2e-*.mjs | wc -l` 對第一個數字；
  `package.json` 的 `e2e` 裡 `node scripts/e2e-*.mjs` 的個數對第二個。
...
- `VALIDATION_REPORT.md`：最近一次風險導向複查與發布證據（**累積保留歷史**，最新一段在檔案開頭）。⚠️ 不要在這裡寫死版號——寫死的那一刻就注定過期，2026-09-23 的複查就是這樣抓到它還寫著 v2.1.79，而檔案第一行已經是 v2.1.80。
...
- `DAY`（全調查時段尖峰）**自 v2.1.64 起不再要求 24 小時**（`peakWindowsFor` 裡 `surveyMinutes` 已改成只決定單位寫「調查日」還是「調查時段」，見 `void surveyMinutes;` 那一行）。算不出來的原因另有：舊紀錄沒有逐時間格資料、或格距組不成整小時。
- 混合時間格**自 v2.1.83 起逐格採用自己的長度**（使用者 2026-09-24 拍板選項②），並列入資料異常清單（`調查格距混用`）；`LAST_CALC_CHANGE_VERSION` 已隨之推進（⚠️ **值以 `lib/traffic.ts` 為準，這裡刻意不寫出來**——理由見 6.5，那一節記載的事故就是「文件寫死一個過期的值，然後反過來要求把正確的改回舊的」；2026-09-25 補正，原本這一行自己就寫死了一個值，而它在那個警告的**前面**）。**再要改口徑仍須使用者明確決策。**
  ⚠️ 2026-09-25 更正這兩句：原文一句說 DAY 必須調查涵蓋滿一整天才提供，另一句說混用格長只警告而不動挑選邏輯、未經授權不得更改——兩句都已與程式相反。第二句尤其危險：它是寫給下一個人的紅線，照它做會把 v2.1.83 的核心修正當成「未經授權順手改掉的口徑變更」而要求還原，而這份文件自己就記錄過一次「文件反過來要求把正確的值改回舊值」的事故。

### 6.3 統計範圍、單位與缺值

- `AM`、`PM`、`DAY` 都是 60 分鐘流率：PCU/hr 或 輛/hr。
- `FULL` 是**整份調查的累計量**：單位由 `scopeUnit()` 依涵蓋範圍決定——整批都是 24 小時才寫
  `PCU/調查日`／`輛/調查日`，不足 24 小時或混著不同涵蓋範圍時寫 `PCU/調查時段`／`輛/調查時段`
  （判斷不出來時也一律寫「調查時段」：寫少不寫多才不會把 4 小時的累計量誤讀成全日量）。
- **`FULL` 不要求完整調查日**：`fullDayUnavailableReason()` 只在「沒有記錄調查時數」時才回原因；
  不足 24 小時照樣算得出來，而 `scopeWindowLabel()` 會把實際涵蓋時數寫出來（例如「4 小時」）。
  ⚠️ 2026-09-25 第五輪複查更正這兩條：原文把 `FULL` 說成必須有完整的一整天才算得出來、
  時數不足就只顯示破折號與原因——**與 v2.1.64 之後的行為相反**，而且與同一節第一條
  （單位依涵蓋範圍二選一）互相矛盾。這是同一節、同一類、同一個成因的第二個實例：
  09-25 那一輪只修了 `DAY`，`FULL` 沒一起修。守門已補上這兩句的樣式。
  （更正註記刻意**轉述**而不引用原句——原句引用會被守門抓出來，本輪我又踩了一次。）
- **`DAY`（全調查時段尖峰）不要求 24 小時**（見 6.2 最後兩行）。只有 AM／PM 的原格式檔案，其 `FULL` 顯示「－」與原因。
  ⚠️ 2026-09-25 更正：原文把兩者寫成同一條規則（兩個都要求滿一整天），與 v2.1.64 之後的行為相反。手冊那一邊有 `tests/manual-matches-system.test.mjs` 守著不可以再講相反的話，這份交接文件卻沒有人守——而它是「工程唯一正式基準」，所以錯得更貴。
...
- `LAST_CALC_CHANGE_VERSION` **不可以自己往前推**，除非本次確實經使用者同意修改 PCU、尖峰、流向、車種歸類或其他計算口徑。
  ⚠️ 這一條原本寫死「必須維持 `v2.1.30`」，而程式早在 v2.1.64 就已經合法推進過——
  於是這份文件反過來要求把一個正確的值改回舊值。**不要在文件裡寫死這個值**，
  它會過期，而過期的文件會讓複查的人（含 AI）把對的改成錯的。
  現行值一律以 `lib/traffic.ts` 的 `LAST_CALC_CHANGE_VERSION` 為準。
  ⚠️ **這裡刻意不再寫出那個值**——2026-09-24 的 F6 獨立複查抓到：上面才剛警告
  「不要在文件裡寫死這個值」，下一行自己就寫死了一個已經過期的（當時文件寫的
  比程式舊了一版）。⚠️ 2026-09-25 的第四輪又抓到同一件事的第二個實例：
  6.2 那一行也寫死了一個值，而它在這個警告的**前面**、更早被讀到。已一併拿掉。
  最近一次推進的原因是使用者拍板把尖峰小時的認定
  從「數格數」改成「累計分鐘數」（格長一致的資料數字不變，格長混用或
  組不成整小時的才會變），詳見 `CHANGELOG.md` 與 `VERSION_HISTORY`。
...
- **混合時間格挑選**：⚠️ **這一條已經作廢，不要照它做。** 它原本主張這件事不必動到
  計算、加個提醒就好；自 v2.1.83 起混合時間格**逐格採用自己的長度**並列入資料異常
  清單（使用者 2026-09-24 拍板），見 6.2。這條紅線留在這裡只是為了記錄它曾經存在
  ——2026-09-25 第五輪複查抓到「同一件事在 6.2 已更正、第 11 節卻還留著相反的紅線」，
  而紅線比一般敘述危險：照它做會把 v2.1.83 的核心修正當成未經授權的口徑變更而要求還原。
  ⚠️ 這一段刻意**轉述**而不引用原句：原句一字不改地留在文件裡，搜尋或掃描的人
  （包含守門）會把它當成現行規則——2026-09-25 第六輪就是這樣被自己的守門抓到的。
...
- `npm run e2e` 流程：83 支 `e2e-*.mjs` 瀏覽器測試（repo 裡共 84 支，另 1 支試用版 smoke 由 `npm run e2e:tryout` 跑），涵蓋轉向圖、名稱、報表、結論、拖放／顯示、重匯、日別、版面、備份、鎖定、趨勢、日期、XLSX 修復、儲存阻擋／IndexedDB、三岔、存在性裁決、revision batch、圖表版面、跨計畫趨勢、主工具列、篩選覆蓋、頁面隔離、尖峰判定、重複資料阻擋、sticky 遮蓋與八叉版面等；另有 2 支種子資料產生器，合計 85 個執行步驟（不含前置 GitHub build）。報告時必須分開描述，不能稱為「85 支瀏覽器 E2E」。
...
### 12.6 v2.1.83 高風險複查與發布驗證

2026-09-26 以 Claude 的 v2.1.83 完整專案 ZIP（SHA-256
...
因 v2.1.80 之後包含尖峰 60 分鐘口徑、格長、持久化、計算、UI 與匯出變更，依高風險規則
檢查完整資料流。v2.1.81、v2.1.82 均未發布。

- GPT 修正 `lengthMinutes` 未從預覽保存到 `sourceIntervals.rows`、混合格長的
  `survey.minutes` 仍用列數乘眾數、兩個型別漏列 `lengthMinutes`，並補上舊紀錄 label 回推、
  只寫起點仍退回眾數及持久化反證。沒有放寬「不足 60 分鐘即資料不足」。
- GPT 修正完整／部分涵蓋混合時的結論草稿單位、車種組成／報告／Excel／UI 四類單位
  呼叫點漏網、兩個 lint 錯誤，以及使字面 `npm test` OOM 的測試資料建構；正式計算版號
  維持 `LAST_CALC_CHANGE_VERSION = v2.1.83`。
- 使用 bundled Node v24.19.0 與 npm 11.6.2 完成乾淨 `npm ci`；字面 `npm test`
  全部通過：`.mjs` 255／255，TypeScript 371 項中 367 通過、4 項因缺特定真實附件條件式略過，
  0 失敗。
- `npm run e2e` 以不得並行的正式順序完成：83 支瀏覽器測試＋2 支種子資料產生器＝
  85 個步驟，退出碼 0。`npm run e2e:tryout` 另行通過。不得將 85 個步驟稱為 85 支 E2E。
- 手冊 v2.1.83 PDF 共 20 頁，已用 Poppler 點陣化逐頁檢視；未見截字、重疊、表格溢出、
  黑塊或空白頁。完整 audit 為 10 項 dev/build 工具鏈警示（4 moderate、6 high、0 critical），
  未執行破壞性自動升級。
- 本輪沒有取得 4 項條件式測試所需的真實附件，也沒有另以桌面 Microsoft Excel 開啟
  本輪匯出；不得把條件式略過或 v2.1.79 的 Office 證據描述成本輪通過。E2E 已驗證
  10 張工作表、OOXML 與原生圖表結構。
...
- v2.1.83 的 4 項特定真實附件測試因候選包未附原檔而條件式略過；合成資料與 E2E 不能冒充真實附件通過。本輪 Excel 已驗 OOXML、10 張工作表與原生圖表結構，但未另以桌面 Microsoft Excel 開啟；手冊 PDF 20 頁則已用 Poppler 點陣化逐頁檢視。
- 本文件只保存可由 Repository、Git、驗證報告及高價值對話決策交叉支持的內容。無法從現行證據可靠重建的早期聊天細節未寫成事實。

### 已知文件差異與待確認事項

- README 的 Excel 工作表數與 15／20／30／60 分鐘格說明已由 v2.1.83 候選一併更新；
  `app/traffic-app.tsx` 的 IndexedDB／序列化註解也已同步現行實作。這三項不再是待辦，
  但未來變更工作表或時間格能力時仍須同步文件與守門。
...
- 截至 v2.1.83 高風險複查後，沒有已知尚未修正且會阻擋使用的功能 Bug。
...
## 驗證報告的日期規則（A8，2026-09-23 三支統一）

**驗證報告與更新說明上的日期，一律以該版 release commit 的 Git 日期為準**
（台北時間，`git log -1 --date=iso-local`），不是撰寫或發布那一天。

⚠️ 為什麼要訂這條：本專案的 `VALIDATION_REPORT.md` 曾經寫 2026-09-10，
而對應的 commit 是 2026-09-09 17:59（台北）——差一天。日後有人拿報告日期
去對 GitHub 上的紀錄，會以為文件對不上程式、或懷疑有沒有夾帶。

規則的三個細節：
1. **跨夜的情況照 commit 走**，不要為了「看起來比較合理」往後挪一天。
2. commit 日期與實際驗證日期真的不同時（例如隔天才補跑完整 E2E），
   **兩個都寫**：「release commit 日期 X；本報告的驗證於 Y 完成」。
3. 三支程式用**同一條規則**，不各自解釋。

## 待修正事項放在哪裡（A7，2026-09-21 使用者定案）

⚠️ **這個 repository 不保存「待修正事項清單」。**

待修正事項一律以**使用者持有的《待修正事項總表》**為準（不在版控裡）。
本 repository 的文件只留三種東西：
1. **已經定案的口徑**（怎麼算、怎麼命名、為什麼）
2. **已決定的禁止事項**（見 `never-revert-contract.mjs` 的不可回頭清單）
3. **證據界線**（哪些事驗過了、哪些沒有，照實寫）

⚠️ **不留「待裁示／尚未處理」這一類分類。** 理由：過期的待裁示段落會讓
下一個維護者（含 AI）把**已經定案**的事當成還沒決定的事，然後「順手」
改掉一個正確的設計。2026-09-21 就發生過一次，是使用者當場擋下來的。

## 「顯示調查日期」開關：現行規則（三支一致，2026-09-21 定案）

> 使用者 2026-09-23：「顯示偏好不影響數值正確性，所以備份有沒帶著走其實都可以，
> 只要日後 AI 維護不論是誰都要知道當前規則是什麼，**不要一個修這樣另一個修別樣**」

**現行規則，三支完全相同：**

| 這個開關 | 會不會跟著走 |
| --- | --- |
| 這台電腦的本機儲存（自動存檔） | ✅ **會**——它是「這台電腦這個人」的偏好 |
| 個人「全部計畫」備份包 | ✅ 會 |
| **單一計畫備份／專案包** | ❌ **不會**（不寫入、也不讀取） |

**為什麼單一計畫備份不帶**：那是拿去給別人、或拿別人的進來用的東西。
夾帶顯示偏好的結果是——併入之後使用者自己的開關被別人的習慣翻掉，
而且畫面上**沒有任何提示**。以交通服務水準的做法為準（它本來就是這樣）。

**三支各自的實作位置：**

- 交通服務水準：`state.showSurveyDate`，整份 state 進 IndexedDB；專案包不含此欄位。
- 路口轉向：進自動存檔；`exportBackupJson()` 以 `...(scoped ? null : {...})` 只在
  全部計畫包寫入；`restoreBackup()` 的**併入**分支不讀它。
- 全日交通量：`localStorage` 的 `traffic-show-survey-date-v1`；讀不到時**預設開**。

⚠️ **這一條不影響任何數值。** 它寫在這裡的唯一目的是：
**三支要一致，而且下一個維護者（含 AI）要看得到現行規則是什麼。**
要改的話三支一起改，並同步更新：
`never-revert-contract.mjs` 第 22 條、各支的備份測試、路口轉向的
`scripts/e2e-backup.mjs`（那裡的斷言曾經是反過來寫的，原處留了警語）。
