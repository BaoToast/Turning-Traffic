...
test("異常檢查會列出「調查日期不只一個」，而且是人工確認", () => {
  ...
  /*
   * ⚠️ 2026-09-24 放寬：原本寫的是 /\| "調查日期不只一個";/——**結尾那個分號**
   *   等於要求「這一類必須是聯集的最後一項」。於是 2026-09-24 依使用者指定
   *   新增「調查格距異常」「調查格距混用」兩類時，這一條紅了，
   *   而紅的理由是「你在後面又加了類別」——那正是當時要做的事。
   *   同一個檔案的 maintenance 守門註解早就寫著這個教訓
   *   （「要守的從來不是有幾種，而是每一種都有解決方式」），這裡漏了。
   *   現在只驗**有登記**，不管排在第幾個。
   */
  assert.match(
    traffic,
