...
7. 重新產生手冊（`node scripts/manual/build-pdf.mjs`），把 `public/` 產出的那一份
   **複製**到**其餘每一個位置**（不要各自重跑，PDF 內嵌產生時間，重跑會得到不同位元組），
   並**刪除**每一個位置下舊版號的 `.pdf`。
   ⚠️ 位置一共有這幾個，**少一個 `tests/manual-copies-identical.test.mjs` 就會紅**
   （它遞迴掃全包，要求每一份逐位元相同）：
   - 倉庫根目錄
   - `public/`（`build-pdf.mjs` 的產出位置）
   - `github-pages-dist/`（發布根目錄的來源）
   - `dist/client/`（只有在本機跑過 `npm run build` 之後才會有；有就要一起換）
   ⚠️ 2026-09-25 補正：這一步原本只寫了「根目錄」與「`public/`」兩個位置，
   而那正是它自己記載過的那個坑（「`public/` 更新了、根目錄忘了同步」）往後移兩格。
...
2. 帶版號的手冊：`路口轉向程式手冊_<版號>.pdf`
   （⚠️ 2026-09-25 更正：這一行原本寫 `Turning-Traffic-<版號>-新手操作手冊.pdf`，
   那是舊檔名。照舊名去抓會 404，然後得到「線上不是最新版」的錯誤結論。）
